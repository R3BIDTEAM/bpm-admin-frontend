...
export class AuthService {
  ...
  public setSession(session: AuthData): void {
    localStorage.setItem('session_' + coreConfig.app.appName, JSON.stringify(session));
  }

  public getSession(): AuthData {
    if (this.isAuthenticated()) {
      return JSON.parse(localStorage.getItem('session_' + coreConfig.app.appName));
    } else {
      return {} as AuthData;
    }
  }
  ...
  public getMenu(): any {
    const sess = JSON.parse(localStorage.getItem('session_' + coreConfig.app.appName));
    if (sess) {
  ...
  public closeSession(): void {
    localStorage.removeItem('session_' + coreConfig.app.appName);
    sessionStorage.clear();
    this.router.navigate(['/']);
  }

  public isAuthenticated(): boolean {
    const session = localStorage.getItem('session_' + coreConfig.app.appName);
