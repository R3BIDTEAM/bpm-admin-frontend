...
export class AltaExpedienteComponent implements OnInit {
    ...
    displayedColumns: string[] = ['conjunto_documental', 'documento', 'obligatorio','entregado'];
    displayedColumnsPromovente: string[] = ['tipo_persona', 'datos_personales', 'datos_identificativos','datos_contacto','notificacion','acciones']
    displayedColumnsRepresentantes: string[] = ['tipo_persona', 'datos_personales', 'datos_identificativos','datos_contacto','notificacion','acciones']
    displayedColumnsCuentas: string[] = ['cuenta', 'direccion', 'acciones']
    ...
    @ViewChild('tablePromoventes') tablePromoventes: MatTable<any>;
    @ViewChild('tableRepresentante') tableRepresentante: MatTable<any>;
    
    @ViewChild('tableCuentasCatastrales') tableCuentasCatastrales: MatTable<any>;
    constructor(
    ...
    addPromoventeFisica(): void {
      if(this.validatePromoventeRepresentante(this.promoventeFisica.value.rfc)){
        ...
        this.tablePromoventes.renderRows();
    ...
    addRepresentanteFisica(): void {
      if(this.validateRepresentantePromovente(this.representanteFisica.value.rfc)){
        ...
        this.tableRepresentante.renderRows();
      ...
      this.http.post(getCuentasCatastralesCurso, filtro, this.httpOptionsContentType).subscribe(
        (res: any) => {
          ...
          if(res.error.code === 0)
          {
            if(res.data.respuesta.length > 0)
            {
              cuentaCatastral.region = res.data.respuesta[0].region;
              cuentaCatastral.manzana = res.data.respuesta[0].manzana;
              cuentaCatastral.lote = res.data.respuesta[0].lote;
              cuentaCatastral.unidadPrivativa = res.data.respuesta[0].unidadPrivativa;
              ...
              this.tableCuentasCatastrales.renderRows();
            } else {
